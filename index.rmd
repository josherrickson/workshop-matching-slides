...
- Note that all of the above still used `x1` and `x2` *even though we matched
  on them*.
    - Balance vs overlap
    - Doubly robust
...
catholic ~ race_white + race_black + race_hispanic +
  ...
  w3momscr + w3dadscr + w3income + w3povrty +
  p5fstamp
```
...
```{r, eval = FALSE}
lmer(c5r2mtsc ~ catholic + race_white + race_black + race_hispanic +
                race_asian + p5numpla + p5hmage + p5hdage +
                w3momscr + w3dadscr + I(w3income/10^3) + w3povrty +
                p5fstamp + (1 | match))
```
